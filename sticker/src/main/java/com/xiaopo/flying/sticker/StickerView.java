...
public class StickerView extends FrameLayout {
    ...
    /**
     * Swaps the specified sticker to the front
     */
    public void bringStickerToFront(Sticker sticker) {
        if (stickers != null) {
            int size = stickers.size();
            if (size > 0) {
                int index = stickers.indexOf(sticker);
                if (index != -1 && index >= 0 && index != size - 1) {
                    swapLayers(index, size - 1);
                }
            }
        }
    }
    ...
    /**
     * Swaps the current sticker to the front
     */
    public void bringCurrentStickerToFront() {
        bringStickerToFront(this.getCurrentSticker());
    }
    ...
    public boolean onTouchEvent(MotionEvent event) {
        ...
        switch (action) {
            ...
            case MotionEvent.ACTION_MOVE:
                boolean intercepted = handleCurrentMode(event);
//                if (intercepted) {
//                    return true;
//                }
                invalidate();
...
//        }
    ...
    protected boolean handleCurrentMode(@NonNull MotionEvent event) {
        ...
        switch (currentMode) {
            ...
            case ActionMode.DRAG:
                if (handlingSticker != null) {
                    ...
                    if (constrained) {
                        intercepted = constrainSticker(handlingSticker);
                        if (intercepted) {
                            //恢复初始状态
                            currentMode = ActionMode.NONE;
                        }
    ...
    protected boolean constrainSticker(@NonNull Sticker sticker) {
        ...
        float stickerWidth = handlingSticker.getCurrentWidth();
        float stickerHeight = handlingSticker.getCurrentHeight();
        if (currentCenterPoint.x < -minEdgeRate * stickerWidth) {
            //left 超出1/4
            intercepted = true;
        } else if (currentCenterPoint.y < -minEdgeRate * stickerHeight) {
            //top 超出1/4
            intercepted = true;
        } else if (currentCenterPoint.x > width && currentCenterPoint.x > width + minEdgeRate * stickerWidth) {
            //right 超出1/4
            intercepted = true;
        } else if (currentCenterPoint.y > height && currentCenterPoint.y > height + minEdgeRate * stickerHeight) {
            //bottom 超出1/4
            intercepted = true;
        }
        if (intercepted) {
            moveX = width / 2 - currentCenterPoint.x;
            moveY = height / 2 - currentCenterPoint.y;
        }
...
//                .postScale(scaleFactor / 2, scaleFactor / 2, getWidth() / 2, getHeight() / 2);
    ...
    protected void setStickerPosition(@NonNull Sticker sticker, @Sticker.Position int position) {
        ...
        float offsetX = 0;
        float offsetY = 0;
        if ((position & Sticker.Position.CENTER) > 0) {
            offsetX = (width - sticker.getWidth()) / 2;
            offsetY = (height - sticker.getHeight()) / 2;
//            offsetY /= 4f;
        }
...
//        else
        if ((position & Sticker.Position.BOTTOM) > 0) {
            offsetY = height - sticker.getHeight();
//            offsetY *= 3f / 4f;
        }
//        else {
//            offsetY /= 2f;
...
//        else
        if ((position & Sticker.Position.RIGHT) > 0) {
            offsetX = width - sticker.getWidth();
//            offsetX *= 3f / 4f;
        }
//        else {
//            offsetX /= 2f;
